...
TEMP_CLIENT_PROPS="$(mktemp "${OUTPUT_DIR}/tls-client.XXXXXX.properties")"
cleanup() {
...
sed "s|__TLS_STORE_PASSWORD__|${TLS_STORE_PASSWORD}|g" \
  deploy/kafka/client/tls-client.properties.template > "${TEMP_CLIENT_PROPS}"
...
run_with_retries "${MAX_RETRIES}" "${RETRY_SLEEP_SECONDS}" remote_ssh "sudo install -m 0640 ${REMOTE_DIR}/$(basename "${TEMP_CLIENT_PROPS}") /etc/kafka/client/tls-client.properties"

log "TLS benchmark client config installed at ${BENCHMARK_CLIENT_IP}"
