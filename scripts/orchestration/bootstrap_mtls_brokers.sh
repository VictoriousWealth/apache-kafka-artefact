...
wait_for_kafka_mtls_api() {
  local host="$1"
  local bootstrap_host="$2"
  local attempt=1
  local remote_cmd="sudo /opt/kafka_2.13-3.8.0/bin/kafka-broker-api-versions.sh --bootstrap-server ${bootstrap_host}:9094 --command-config /etc/kafka/tls/mtls-client.properties >/dev/null 2>&1"
  while (( attempt <= MAX_RETRIES )); do
...
TEMP_MTLS_CLIENT_PROPS="$(mktemp "${OUTPUT_DIR}/broker-mtls-client.XXXXXX.properties")"
cleanup() {
  rm -f "${TEMP_CLUSTER_ID}" "${TEMP_MTLS_CLIENT_PROPS}"
}
trap cleanup EXIT

sed "s|__TLS_STORE_PASSWORD__|${TLS_STORE_PASSWORD}|g" \
  deploy/kafka/client/mtls-client.properties.template |
  sed "s|/etc/kafka/client/tls/kafka.client.truststore.p12|/etc/kafka/tls/kafka.server.truststore.p12|g" |
  sed "s|/etc/kafka/client/tls/kafka.client.keystore.p12|/etc/kafka/tls/kafka.server.keystore.p12|g" \
  > "${TEMP_MTLS_CLIENT_PROPS}"

for i in "${!BROKER_PUBLIC_IPS[@]}"; do
  HOST="${BROKER_PUBLIC_IPS[$i]}"
  NODE_ID=$((i + 1))

  log "Preparing mTLS broker host ${HOST}"
  ...
  run_with_retries "${MAX_RETRIES}" "${RETRY_SLEEP_SECONDS}" remote_scp_to "${HOST}" \
    ...
    deploy/kafka/config/server.properties.mtls.template \
  ...
  run_with_retries "${MAX_RETRIES}" "${RETRY_SLEEP_SECONDS}" remote_ssh "${HOST}" "sudo install -m 0640 -o kafka -g kafka ${REMOTE_DIR}/$(basename "${TEMP_MTLS_CLIENT_PROPS}") /etc/kafka/tls/mtls-client.properties"
  run_with_retries "${MAX_RETRIES}" "${RETRY_SLEEP_SECONDS}" remote_ssh "${HOST}" "sudo install -m 0644 -o kafka -g kafka ${REMOTE_DIR}/server.properties.mtls.template /etc/kafka/server.properties.template"
done
...
for i in "${!BROKER_PUBLIC_IPS[@]}"; do
  ...
  log "Configuring mTLS broker ${NODE_ID} at ${HOST}"
...
for i in "${!BROKER_PUBLIC_IPS[@]}"; do
  ...
  log "Restarting mTLS broker ${NODE_ID} at ${HOST}"
...
for i in "${!BROKER_PUBLIC_IPS[@]}"; do
  ...
  if ! wait_for_kafka_service "${HOST}"; then
    log "Kafka mTLS service failed health check on ${HOST}"
    exit 1
  fi
...
for i in "${!BROKER_PUBLIC_IPS[@]}"; do
  HOST="${BROKER_PUBLIC_IPS[$i]}"
  KAFKA_HOST="${BROKER_PRIVATE_IPS[$i]}"
  log "Checking Kafka mTLS API readiness on ${KAFKA_HOST}:9094"
  if ! wait_for_kafka_mtls_api "${HOST}" "${KAFKA_HOST}"; then
    log "Kafka mTLS API readiness check failed on ${KAFKA_HOST}:9094"
    exit 1
  fi
done

echo "mTLS broker bootstrap complete."
